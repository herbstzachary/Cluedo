...
class GameStateInformationArea:
    ...
    def __init__(self, area, font):
        ...
        self.suggest_text = font.render("Submit Suggestion", True, BLACK)
        ...
        self.accuse_text = font.render("Accuse", True, BLACK)
        ...
        self.skip_accuse_text = font.render("Skip Accuse Phase", True, BLACK)
    ...
    def draw_info_area(self, player, phase, move_amount, guess, surface):
        if phase == TurnPhases.MOVE:
            ...
            if self.eliminated_player is not None:
                eliminated_text = self.font.render(self.eliminated_player.character.value + " was eliminated.", True, BLACK)
            ...
            text = self.font.render(player.character.value + " can move up to " + str(move_amount) + " spaces", True, BLACK)
            ...
            if None in guess.values():
                pygame.draw.rect(surface, GREY, text_rect)
            else:
                pygame.draw.rect(surface, GREEN, text_rect)
            pygame.draw.rect(surface, BLACK, text_rect, width=1)
            surface.blit(self.suggest_text, text_rect)
        else:
            if None in guess.values():
                pygame.draw.rect(surface, GREY, self.accuse_submit_button)
            else:
